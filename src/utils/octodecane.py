...
smoothed_solid_paths = sorted(
    glob.glob("../data/octodecane/smoothed_solid/u_*.npz"),
    key=lambda f: int(re.search(r"u_(\d+)", f).group(1)),
)
img = plt.imread("../data/okada.png")
...
thermal_params = ThermalParameters.load_from_file(
    "./parameters/octodecane/thermal_params.json"
)
fluid_params = FluidParameters.load_from_file(
    "./parameters/octodecane/fluid_params.json"
)
...
for file_path in smoothed_solid_paths:
    ...
    if match:
        ...
        ax.plot(X_b, Y_b, linestyle="--", color="green", linewidth=2)
...
smoothed_solid = mlines.Line2D(
    ...
    color="pink",
    linewidth=2,
    label=r"Размазывание в твердой фазе",
)

ax.legend(handles=[smoothed_solid])
plt.show()
