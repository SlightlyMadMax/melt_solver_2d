    ...
    def solve(
        ...
        for i in range(self.implicit_lin_max_iters):
            ...
            self._compute_sweep_x(
                ...
                right_value=(
                    self.bcs.right.get_value(t=time - 0.5 * self.geometry.dt)
                    if self.bcs.right.boundary_type == BoundaryConditionType.DIRICHLET
                    else None
                ),
                left_value=(
                    self.bcs.left.get_value(t=time - 0.5 * self.geometry.dt)
                    if self.bcs.left.boundary_type == BoundaryConditionType.DIRICHLET
                    else None
                ),
                right_flux=(
                    self.bcs.right.get_flux(t=time - 0.5 * self.geometry.dt)
                    if self.bcs.right.boundary_type == BoundaryConditionType.NEUMANN
                    else None
                ),
                left_flux=(
                    self.bcs.left.get_flux(t=time - 0.5 * self.geometry.dt)
                    if self.bcs.left.boundary_type == BoundaryConditionType.NEUMANN
                    else None
                ),
                right_psi=(
                    self.bcs.right.get_psi(t=time - 0.5 * self.geometry.dt)
                    if self.bcs.right.boundary_type == BoundaryConditionType.ROBIN
                    else None
                ),
                left_psi=(
                    self.bcs.left.get_psi(t=time - 0.5 * self.geometry.dt)
                    if self.bcs.left.boundary_type == BoundaryConditionType.ROBIN
                    else None
                ),
                right_phi=(
                    self.bcs.right.get_phi(t=time - 0.5 * self.geometry.dt)
                    if self.bcs.right.boundary_type == BoundaryConditionType.ROBIN
                    else None
                ),
                left_phi=(
                    self.bcs.left.get_phi(t=time - 0.5 * self.geometry.dt)
                    if self.bcs.left.boundary_type == BoundaryConditionType.ROBIN
                    else None
                ),
            )
